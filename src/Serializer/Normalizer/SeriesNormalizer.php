 ...
 */
final class SeriesNormalizer extends AbstractNormalizer
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): array
    {
        if (! empty($data['amiibo']) && ! isset($data['amiibo'][0])) {
            return [Series::createFromArray($data['amiibo'])];
        }

        return array_map(function (array $seriesData) {
            return Series::createFromArray($seriesData);
        }, $data['amiibo'] ?? []);
    }
